...
required_columns = ['PERIODO', 'CO_CLIENTE', 'NU_PRESTAMO', 'TIPO_CLIENTE', 'GRUPO0', 'CO_MONEDA', 'FE_DESEMBOLSO', 'MO_DESEMBOLSO_SOLES', 'SEG_RIE', 'CANAL', 'PLAZO_MESES', 'TEA','TIR_OBJ']
...
dict_comp = ["saldo","tea","tt","saldo","capital","ingreso_financiero","costo_financiero","perdida_esperada"
            ,"rem_capital","desgravamen","costos","imp_renta","flujo_neto","van","tir","roe"]
...
inputs = pd.read_csv(full_filename, nrows=n_rows,sep=dialect.delimiter,encoding='ISO-8859-1')
...
if is_string_dtype(inputs2.MO_DESEMBOLSO_SOLES) and inputs2.MO_DESEMBOLSO_SOLES.str.find('%').all():
...
if not (is_numeric_dtype(inputs2.PERIODO) and inputs2.PERIODO.astype(str).str.len()==6).all():
...
inputs2.FE_DESEMBOLSO.fillna(pd.to_datetime(inputs.PERIODO, format='%Y%m'),inplace=True)
...
if not inputs2.GRUPO0.isin(['Activo Fijo',	'Capital de Trabajo',	'RURAL',	'Hipotecario',	'Construccion de Vivienda',	'Lineas',	'Consumo' ,'Compra Deuda']).all():
...
inputs_it = pd.read_csv("D:/CLV_Original/data/input/"+data_filename+"_clean.csv", nrows=n_rows, chunksize=chunk_size, iterator=True)
inputs = inputs2
