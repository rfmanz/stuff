    ...
    try:
        ...
        with open(abs_sql_file) as sql:
            ...
            if tmp and tmp[len(tmp) - 1] != '.DS_Store':
                ...
                logging.info('Existing TSV file %s being reused %.4fs' % (file, get_time(start_time)))
            else:
                # process records using server-side cursor
                file = get_tmp_file()
                with connection.cursor() as cursor:
                    with open(file, 'wb') as tmp:
                        logging.info('Executing copy statement for records in %.4fs' % get_time(start_time))
                        cursor.copy_expert(copy, tmp)
                        logging.info('Records copied from Postgres in %.4fs' % get_time(start_time))
                        
            with open(file, 'r') as item:
                total = sum(1 for _ in item)
                logging.info('Processing %s records from file in %.4fs' % (str(total), get_time(start_time)))
                
                
            with open(file, 'r') as tmp:
                logging.info('Reading TSV from Postgres records in %.4fs' % get_time(start_time))
                csv.field_size_limit(sys.maxsize)
                tsv = csv.DictReader(tmp, delimiter='\t')
                logging.info('Records being parsed from TSV in %.4fs' % get_time(start_time))
                ...
                logging.info('total record={}'.format(len(df)))
            ...
            logging.info('%s records in %.4fs' % (count, get_time(start_time)))
            os.remove(file)
            return df

    except:
        logging.info('Error: %s records in %.4fs' % (count, get_time(start_time)))
...
def main():
    ...
    #logging.warning('SQL file path={} '.format(sql_dir))
    logging.info('SQL file path={} '.format(sql_dir))
