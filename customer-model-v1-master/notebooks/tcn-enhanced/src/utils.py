...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, model_object):
        ...
        model_object: 
            the high-level TCNBinaryClassifier object.
            need to have a .save module.
...
def parse_meta(meta, seq_len, return_sequences=True, min_length=1):
    """ parse meta data to the format needed 
        
    Parameters
    ----------
    meta: list of list/array
        meta data for each data point
        
        
    seq_len: list of int
        sequence length for the source being fed into TCN corresponds to each meta data
    ...
    ----------
    result: np.array
        meta data with each row for a datum at an individual time step.
...
def parse_pred(pred, seq_len, return_sequences=True, min_length=1, flatten=True):    
    """  
    ...
    seq_len: list
        length of the source sequence for each prediction
    ...
    flatten: bool, default = True
        Whether to flatten result. Only makes sense when dealing with scalar predictions.
    ...
    if len(shape) == 3: # pred is probability
        b, s, f = shape
        if f > 1 and flatten:
            raise ValueError("Doesn't make sense to flatten probability")
...
class Timer: 
    ...
    def time_since_start(self):
        """
        return the time passed since initializing the Timer
        """
        if self.is_timer_paused:
            self.resume()
            self.is_timer_paused = False
...
class LossGenie(object):
    ...
    def __init__(self, criterion, **kwargs):
        ...
        This wrapper object provides cumulative 
...
class InferenceFormatter:
    ...
    def get_inference(self, meta_columns=None):
        """
        ...
        ----------
        meta_columns: list
            meta data columns in stored order
        
        
        Returns
        ----------
        results: pd.DataFrame or np.array
            if meta_columns are provided, result is a dataframe with meta columns along with tcn_pred
