...
def correlated(df, threshold, drop_columns=False, encode_type="dmy"):
    ...
    to_drop = pd.Series(
        df_corr.iloc[
            ...
            np.where(
                (
                    df_corr.mask(np.tril(np.ones(df_corr.shape, dtype=bool))).abs()
                    > threshold
                ).any()
            )[0],
        ].columns
...
def mice_imputer(data):
    ...
    # Create kernel.
    kds = mf.KernelDataSet(na_cols, save_all_iterations=True, random_state=1991)
...
def target_encode(train, valid, col, target="target", kfold=5, smooth=20, verbose=True):
    ...
    for i in range(kfold):
        ...
        df_tmp = (
            df_tmp[col + [target]].groupby(col).agg(["mean", "count"]).reset_index()
        )
        ...
        df_tmp_m = (
            train[col + ["kfold", "org_sorting", "TE_" + col_name]]
            .merge(df_tmp, how="left", left_on=col, right_on=col)
            .sort_values("org_sorting")
        )
        df_tmp_m.loc[df_tmp_m["kfold"] == i, "TE_" + col_name] = df_tmp_m.loc[
