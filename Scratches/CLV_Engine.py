...
class SetPath():
    ...
    def set_path_can(self):
        ...
        param_Can = pd.read_csv(str(p1),sep="|",float_precision = "round_trip",dtype=np.float64,converters={"Producto":str})
        hyperparams = {
             "param_Can": param_Can
        }     
    ...
    def set_path_pd(self):
        ...
        param_pd1 = pd.read_csv(str(p1),sep="|",float_precision = "round_trip",dtype=np.float64,converters={"Segmento De Riesgo":str,"Producto":str})
        param_pd2 = pd.read_csv(str(p2),sep="|",float_precision = "round_trip",dtype=np.float64,converters={"Segmento De Riesgo":str})
        hyperparams = {
             "param_pd1": param_pd1,
             "param_pd2": param_pd2
        }
    ...
    def set_path_pre(self):
        ...
        param_pre = pd.read_csv(str(p1),sep="|",float_precision = "round_trip",dtype=np.float64,converters={"Segmento De Riesgo":str,"Producto":str})
        hyperparams = {
             "param_pre": param_pre
        }
    ...
    def set_path_cost(self):
        ...
        hyperparams = {
            "param_t0": param_costo_t0,
            "param_t1T": param_costo_t1T
        }
    ...
    def set_path_mapSegRisk(self):
        ...
        hyperparams = {
            "param_mapSegRisk":param_mapSegRisk
        }
...
class CLVEngine():
    ...
    def uniformize_input(self,X):
        ...
        X = pd.merge(X,segInst,left_on="PLAZO_MESES",right_on="installment",how="left")
    ...
    def transform(self, X):
        ...
        self.X_tr["descuentos"], self.X_tr["desembolsos"], self.X_tr["r"], self.X_tr["T"] = clv.get_parameters_transform(
    ...
    def predict(self):
        ...
        for key in self.curves:
            ...
            self.curves[key] = self.curves_instance[key].predict(
                self.X_tr[key])
        return self.curves
    ...
    def get_rmin(self):
        ...
        def funct(x): return clv.van_cronograma(
            self.X_tr["descuentos"],
            self.X_tr["desembolsos"],
            x,
            self.X_tr["T"],
    ...
    def get_decomposition_leads(self, tasas_propuestas, rmins_mensual):
        ...
        tasas_mensual = np.power((1 + tasas_propuestas), 1/12) - 1 # Mensualizando la tasa propuesta
        ...
        rmin_decomp_df = self.get_rmin_decomp(r=rmins_mensual) # Recibe una rmin mensual
        ...
        df.loc[:, "Tasa Propuesta"] = tasas_propuestas # Tasas Propuestas luego de aplicar reglas de negocio
        df = pd.concat([df, rmin_decomp_df], axis=1) # Descomposicion de la Tmin + Saldo y Capital Promedio
        df.loc[:, "Desembolsos"] = self.X_tr["desembolsos"]
        df.loc[:, "Tasas Ponderadas"] = df.loc[:, "Desembolsos"].values*tasas_propuestas # Tmin Ponderada por Desembolsos
        df.loc[:, "TIR Ponderada"] = df.loc[:, "Capital Promedio"].values*tirs # TIR Ponderada por Capital Promedio
    ...
    def get_profitability_indicators(self):
        ...
        df = pd.concat([pvs_df, tirs_df, rmin_df, rmin_decomp_df], axis=1) # Concatenando los DataFrames
    ...
    def get_components(self,dict_comp,r_calc):
        ...
        df = clv.get_components(self.X_tr["descuentos"],
            self.X_tr["desembolsos"],
            r,
            self.X_tr["T"],
            self.param,dict_comp)#.reshape(self.m,)
        return df
    ...
    def get_components_validation(self,dict_comp,r_calc):
        ...
        df = clv.get_components_validation(self.X_tr["descuentos"],
            self.X_tr["desembolsos"],
            r,
            self.X_tr["T"],
            self.param,dict_comp)
        return df
    ...
    def get_components_validation2(self,dict_comp,r_calc):
        ...
        df = clv.get_components_validation2(self.X_tr["descuentos"],
            self.X_tr["desembolsos"],
            r,
            self.X_tr["T"],
            self.param,dict_comp)
