WITH last_unrestricted_date AS
    ( SELECT business_account_number,
     ...
     WHERE business_account_number in <BUSINESS_ACCOUNT_NUMBER>
         AND account_restricted_ind=FALSE
    ...
    ( SELECT business_account_number,
     ...
     FROM banking_accounts
     WHERE business_account_number in <BUSINESS_ACCOUNT_NUMBER>
         AND account_restricted_reason LIKE '%%No%%'
     GROUP BY 1 )
...
WHERE ba.business_account_number in <BUSINESS_ACCOUNT_NUMBER>
GROUP BY 1