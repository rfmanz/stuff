...
def remove_corr_fts(df, features, weight_col, corr_threshold=0.8):
    corr_matrix = WeightedCorr(df=df[features+[weight_col]], 
...
class FeatureSelector(object):
    ...
    def get_mlp_shap(self, train_df, valid_df, features, target_col, 
                     weight_col, mlp_params, nsamples=1000):
        from tensorflow import keras
        from src.architectures import get_mlp_clf, get_initial_bias

        n_classes = 2
        
        y_train = keras.utils.to_categorical(train_df[target_col], n_classes)
        y_valid = keras.utils.to_categorical(valid_df[target_col], n_classes)
        
        output_bias = get_initial_bias(train_df[target_col], 
                                       train_df[weight_col])

        mlp = get_mlp_clf(num_features=mlp_params["n_features"],
                    num_classes=mlp_params["n_classes"],
                    nhids=mlp_params["nhids"],
                    output_bias=output_bias)
        stop_early = keras.callbacks.EarlyStopping(monitor='val_loss', 
                                                      patience=mlp_params["patience"],
                                                      restore_best_weights=True)
        lr_schedule = keras.optimizers.schedules.ExponentialDecay(
                        initial_learning_rate=mlp_params["lr"],
                        decay_steps=mlp_params["lr_decay_steps"],
                        decay_rate=mlp_params["lr_decay_rate"],
                    )
        mlp.compile(optimizer=keras.optimizers.Adam(learning_rate=lr_schedule),
                          loss="CategoricalCrossentropy",
                          metrics=['AUC', 'accuracy']) 
        
        mlp.fit(train_df[features],
            y_train,
            batch_size=mlp_params["batch_size"],
            epochs=mlp_params["epochs"],
            validation_data=(valid_df[features],
                             y_valid,
                             valid_df[weight_col].values),
            callbacks=[stop_early],
            sample_weight=train_df[weight_col].values,
            use_multiprocessing=True)
        
        df_ = train_df.sample(n=nsamples, random_state=42)
        explainer = shap.DeepExplainer(model=mlp, data=df_[features].values)
        shap_values = explainer.shap_values(df_[features].values)
        shap_features = get_feature_shap_abs(shap_values[1], features)

        return mlp, shap_values, shap_features
