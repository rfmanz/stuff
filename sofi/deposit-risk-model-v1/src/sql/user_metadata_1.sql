...
FROM bid
    JOIN TDM_RISK_MGMT_HUB.modeled.banking_account_daily_facts badf ON badf.borrower_id = bid.borrower_id
    JOIN
    (SELECT borrower_id,
        max(reporting_date_id) AS reporting_date_id
    FROM TDM_RISK_MGMT_HUB.modeled.banking_account_daily_facts
    GROUP BY 1) bc ON badf.borrower_id = bc.borrower_id
        AND badf.reporting_date_id = bc.reporting_date_id
    LEFT JOIN TDM_RISK_MGMT_HUB.modeled.banking_accounts ba ON ba.banking_account_id=badf.banking_account_id
    LEFT JOIN TDM_RISK_MGMT_HUB.modeled.borrowers_pii_file bf ON bf.borrower_id = bid.borrower_id
    LEFT JOIN TDM_RISK_MGMT_HUB.modeled.borrowers b ON b.borrower_id = bid.borrower_id
