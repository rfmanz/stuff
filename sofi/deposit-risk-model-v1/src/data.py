...
def query_data():
    ...
    for name, qinf in CONFIG_FILE['sql_query_files'].items():
        ...
        if 'format' in qinf:
            query = query.format(qinf['format'])
            
        raw_data[name] = query_postgres(query=query,
                                        port=qinf['port'],
                                        database=qinf['database'])
    return raw_data
...
def process_raw_data(raw_data):
    ...
    ### Dedup metadata
    metadata_df = metadata_df.sort_values(by=['borrower_id', 'account_open_date'], ascending=False)
    metadata_df = metadata_df.drop_duplicates(subset=['borrower_id']) #keep most recently opened account.
    ...
    transactions_df['time_of_day'] = transactions_df['time_of_day'].astype(str).replace('None', '00:00:00')
    transactions_df['transaction_datetime'] = pd.to_datetime((transactions_df['transaction_created_date_id'].astype(str) +\
                                                              transactions_df['time_of_day']), 
                                                             format='%Y%m%d%H:%M:%S', errors='coerce')
    
    ### Join metadata, credit, and socure to transactions.    
    # Join dataframes
    transactions_df = pd.merge(transactions_df, metadata_df, how='inner', on='borrower_id')
    ...
    transactions_df = pd.merge_asof(transactions_df, credit_df, left_on='transaction_datetime', 
                                    right_on='credit_pull_date', by='borrower_id')
    transactions_df = pd.merge_asof(transactions_df, socure_df, left_on='transaction_datetime',
                                    right_on='socure_pull_date', by='borrower_id')
    ...
    def match_return_to_deposit(banking_transaction_details_id, transactions_df):
        ...
        """
        transaction = transactions_df[transactions_df['banking_transaction_details_id'] == banking_transaction_details_id]
        #reduce to transactions made by this borrower
        ttmp = transactions_df[transactions_df['borrower_id'].isin(transaction['borrower_id'])]
        #transactions made on or before the return date
        ttmp = ttmp[ttmp['transaction_datetime'] <= transaction['transaction_datetime'].iloc[0]] 
        ...
        ttmp = ttmp[ttmp['transaction_amount'] == -1 * transaction['transaction_amount'].iloc[0]] 
    ...
    for transaction_id in tqdm(transactions_df[transactions_df['is_return']]['banking_transaction_details_id'].values):
    ...
    transactions_df = pd.merge(transactions_df, transactions_df.groupby('borrower_id')['borrower_id']\
                               .count().rename('num_transactions_all_time').to_frame(), 
                               how='left', on='borrower_id')
    transactions_df['is_indeterminate'] = (transactions_df['num_transactions_all_time'] < 3) |\
                                          (transactions_df['target'] & (transactions_df['latest_acc_bal'] > 0)) |\
                                          (~transactions_df['target'] & (transactions_df['account_closed_reason']\
                                                                         .isin(['Closed by SoFi - Charge-Off / Write-Off', 
...
def get_modeling_dataframe(processed_df):
    ...
    processed_df = processed_df[(processed_df['transaction_code'].isin(['ACHDD', 'DDCK'])) &\
    ...
    processed_df['account_open_date'] = pd.to_datetime(processed_df['account_open_date'])
    processed_df = processed_df[processed_df['account_open_date']\
                                .between(pd.to_datetime('2019-01-01'),
...
def _to_s3(prefix, base_path='data'):
    ...
    for name, path in CONFIG_FILE['data'][prefix].items():
        ...
        upload_s3(bucket_name=CONFIG_FILE['s3_bucket'], path_local=path,
...
def main():
    ...
    Main function.
    """
    parser = argparse.ArgumentParser(description='Command line interface to data manipulation functions.',
                                     prefix_chars='-')
    parser.add_argument('-a', '--all', action='store_true', default=False,
                        help='Run end to end data pipeline.', dest='ga')
    parser.add_argument('-r', '--raw', action='store_true', default=False,
                        help='Pull raw data files.', dest='gr')
    parser.add_argument('-p', '--processed', action='store_true', default=False,
                        help='Build processed data file.', dest='gp')
    parser.add_argument('-f', '--features', type=str, default=None,
                        help='Build features from processed data Pass path to processed data file.', dest='gf')
    parser.add_argument('-m', '--modeling', type=str, default=None,
                        help='Convert processed data file to modeling data file. Pass path to processed data file.', dest='gm')
    parser.add_argument('-b', '--base_dir', type=str, default='data',
                        help='Base directory to which files should be saved.', dest='bd')
    parser.add_argument('-u', '--upload_s3', type=str, default=None,
