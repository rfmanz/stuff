...
def partial_dependency_(model: lgb.Booster, df: pd.DataFrame, 
                        feature: str, features: list,
                        num_cuts=20, include_missing=True, 
                        missing_idx=-1):
    """
    ...
    df_["bins"] = pd.cut(df_[feature], cutoffs, right=True,
...
def pdp_w_impact_table(model: lgb.Booster, df: pd.DataFrame, 
                       features: list, num_cuts=20, include_missing=True):
    """
    providing a fixed feature, compute partial dependency by value quantiles
    ...
    for f in tqdm(features):
        tbl_ = partial_dependency_(model, df, f, features,
                                   num_cuts=num_cuts, include_missing=include_missing)
...
def plot_pdp_w_impact(pdp_dict, features, ncols=3, figsize=None, 
                      linewidth=3, fig=None, axs=None, max_n_ticks=15, **kwargs):
    """
...
def build_aa_code_pdp(df: pd.DataFrame, pdp_dict: dict, 
                      aa_df: pd.DataFrame, inq_fts: list=[],
    ...
    impact_df.loc[impact_df["is_inquiry"]
                  & (impact_df["impact"] > 0), "inquiry_impact"] = 1
    
    pos_impact = impact_df[impact_df.impact > 0]
    pos_impact_aa = pos_impact.merge(aa_df, how="left", on="attr")

    pos_impact_aa.sort_values(by=["inquiry_impact", "impact"], ascending=False, inplace=True)
    
    n = 4
    if impact_df["inquiry_impact"].max() >= 1:
        n = 5
    
    pos_impact_aa['aa'] = np.where(pos_impact_aa.attr.isin(special_list),
