...
def build_table_3(modeling_df, percentiles, target_col_name, pred_col_name, fico_col_name):
    ...
    for (fthresh, pctl), (mthresh, pctl) in zip([(np.percentile(modeling_df[fico_col_name], pctl), pctl) for pctl in percentiles], [(np.percentile(modeling_df[pred_col_name], pctl), pctl) for pctl in percentiles][::-1]):
        mbr = modeling_df[modeling_df[pred_col_name] >= mthresh][target_col_name].mean() * 100
        fbr = modeling_df[modeling_df[fico_col_name] <= fthresh][target_col_name].mean() * 100
...
def build_table_bl(modeling_df, percentiles, target_col_name, pred_col_name, fico_col_name):
    """ Thomas's table 3... model vs fico """
    df = []
    for (fthresh, pctl), (mthresh, pctl) in zip([(np.percentile(modeling_df[fico_col_name], pctl), pctl) for pctl in percentiles][::-1], [(np.percentile(modeling_df[pred_col_name], pctl), pctl) for pctl in percentiles][::-1]):
        mbr = modeling_df[modeling_df[pred_col_name] >= mthresh][target_col_name].mean() * 100
        fbr = modeling_df[modeling_df[fico_col_name] >= fthresh][target_col_name].mean() * 100

        df.append([pctl, fthresh, mthresh, fbr, mbr])
        
    return pd.DataFrame(df, columns=['Percentile', 'Baseline Threshold', 'Model Score Treshold', 'Baseline Bad Rate (%)', 'Model Bad Rate (%)'])
...
def get_model_vs_baseline(
    ...
    table = build_table_bl(df, percentiles, target_col,
                          model_col, baseline_col)
    table.round(2).to_csv(os.path.join(dir_path, "model-vs-baseline.csv"), index=False)
