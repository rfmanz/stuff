...
def get_score_alignment_table(df, src_pred, tgt_pred, target_col,
                              src_pred_range=(0,1), tgt_pred_range=(0,1),
    ...
    br = get_bad_rate(df, tgt_pred, target_col).rename(tgt_br_col)
    df = pd.merge(df, br, left_index=True, right_index=True)

    # bad rate for source prediction - 20191231
    br = get_bad_rate(df, src_pred, target_col).rename(src_br_col)
    df = pd.merge(df, br, left_index=True, right_index=True)
    ...
    br_tbl_tgt = br_tbl_tgt.sort_values(by=tgt_pred).drop_duplicates(tgt_br_cummax, keep="first")

    # cumulative bad rate table for the target prediction
    br_tbl_src = df[[src_pred, src_br_cummax]]
    br_tbl_src[src_br_cummax] = br_tbl_src[src_br_cummax].round(br_precision)
    br_tbl_src = br_tbl_src.sort_values(by=src_pred).drop_duplicates(src_br_cummax, keep="first")
    
    # combine cummax table
    br_tbl = pd.merge(br_tbl_tgt,
                  br_tbl_src,
    ...
    br_tbl = br_tbl.loc[br_tbl[tgt_pred] != br_tbl[src_pred], :]  # do we have to keep this?
...
def get_aligned_score(df, score_alignment_table,
                      src_col, tgt_col, pred_precision=3):
    """
