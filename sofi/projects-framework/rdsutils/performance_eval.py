...
def model_score_eval(Y_true,model,score=[],score2=[],Y_weight=[],decile=10, if_plot = 1):
    ...
    if len(model)==len(Y_true):
        ...
        if if_plot == 1:
            ...
            model_table_t=model_table[['pct_cum_acct','pct_cum_target','actual_rate','cum_bads','cum_acct']]
            model_table_t.columns=['pct_cum_acct',model.name+'_pct_cum_target',model.name+'_actual_rate',model.name+'_cum_bads','cum_acct']
            ...
            ax4.plot(table_total['cum_acct'],table_total[model.name+'_cum_bads'],label=model.name)
