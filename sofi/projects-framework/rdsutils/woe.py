...
class WOE_Transform(object):
    ...
    # initiate values
    def __init__(self,method="tree",num_bin_start=40,min_samples_leaf=500,min_iv=0.01):
        self.method=method
    ...
    def _check(self,X,Y):
        ...
        char_list=dtype[(dtype=='object') | (dtype=='bool')| (dtype=='category')].index.to_list()
        time_list=dtype[(dtype=='datetime64[ns]') | (dtype=='datetime64')].index.to_list()
        
        if time_list:
            print(Fore.GREEN+"datetime attributes are removed from this step:",Style.RESET_ALL)
            print(time_list)
            print()
        ...
        if num_missing_list:
            print(Fore.GREEN+"Attrs removed--missing pct>99%: ",Style.RESET_ALL, end=" ")
    ...
    def _check_transform(self,X):
        ...
        if set(self.num_list).issubset(set(num_list))==True and set(self.char_list).issubset(set(char_list))==True:
            a=1
            
        elif set(num_list).issubset(set(self.num_list))==True and set(char_list).issubset(set(self.char_list))==True:
            a=1
    ...
    def _bin_char(self,X,Y,Y_weight):        
        ...
        for char in self.char_list_local:          
            ...
            if XX[char].nunique()>100:
                ...
                print(Fore.RED+char,Style.RESET_ALL," has more than 100 categories, will be ignored")              
            else:    
                ...
                XX[char+"_t"]=XX[char].apply(lambda x: "0:Other" if x in lessmin_list else x)
                ...
                num_good=round(df_combine[df_combine['target']==0].groupby('x')['weight'].sum(),1)
                num_bad=round(df_combine[df_combine['target']==1].groupby('x')['weight'].sum(),1)
                   
                dist_good=num_good/df_combine[df_combine['target']==0]['weight'].sum()
                dist_bad=num_bad/df_combine[df_combine['target']==1]['weight'].sum()
                target_rates=num_bad/num_accts
                woe=np.log(dist_bad/dist_good)
                iv=(dist_bad-dist_good)*woe
                
                final_bin=pd.concat([num_accts,pct_accts,num_good,num_bad,dist_good,dist_bad,target_rates,woe,iv]
                                    ,axis=1,sort=True)
                                
                final_bin.columns=['#accts','%accts','#good','#bad','dist_good','dist_bad','target_rate',
                                  'woe','iv']
                ...
                final_bin=final_bin[['attr','value','#accts','%accts','#good','#bad',
                ...
                mapping=XX[[char,char+"_t"]].drop_duplicates(keep='first',inplace=False)
                woe_result=mapping.merge(final_bin[['value','woe','%accts']],how='left',left_on=char+"_t",right_on='value')
    ...
    def _bin_num(self,X,Y,Y_weight,speical_value=[]):
        ...
        for name in self.num_list_local:
            ...
            YXX_normal=YXX[(YXX[name].isin(speical_value)==False) & (YXX[name].isnull()==False)]
            ...
            if self.method=="equal":
                ...
                if n_uniq<=self.num_bins:
                    ...
                    nn=self.num_bins
                list1=pd.qcut(YXX_normal[name],nn,retbins=True,labels=False,duplicates="drop")[1].tolist()
                
                self.cutoffs[name]=list1[1:len(list1)-1]
            
            # find cutoff by tree
            if self.method=="tree":
                estimator = DecisionTreeClassifier(min_samples_leaf=self.min_samples_leaf,max_leaf_nodes=self.num_bins)
                estimator.fit(YXX_normal[[name]],YXX_normal['target'],sample_weight=YXX_normal['weight'].values)
                dtree=estimator.tree_
                
                def tree_recurse(node):
                    if dtree.feature[node]!=-2:
                        threshold=dtree.threshold[node]
                        if dtree.feature[dtree.children_left[node]]==-2 or dtree.feature[dtree.children_right[node]]==-2:
            ...
            while abs(round(corr_n,4))<1:
                ...
                bin2=pd.cut(YXX_normal[name],self.cutoffs[name],right=True,labels=False)
                weight_bin=YXX_normal.groupby(bin2)['weight'].sum()
                target_rate=round(YXX_normal.groupby(bin2)['target_weight'].sum()/YXX_normal.groupby(bin2)['weight'].sum(),4)
                #print(target_rate)
                
                corr,p=stats.spearmanr(YXX_normal.groupby(bin2)[name].mean(),target_rate)
                corr_w=wpearson(YXX_normal.groupby(bin2)[name].mean(),target_rate,weight_bin)
            ...
            num_bad=round(YXX_normal[YXX_normal['target']==1].groupby(bin2)['weight'].sum(),1)
            num_good=round(YXX_normal[YXX_normal['target']==0].groupby(bin2)['weight'].sum(),1)
            
            #num_bad=0 if num_bad is None
            #num_good=0 if num_good is None
        
            target_rate=num_bad/num_accts
            normal=pd.concat([min_v,max_v,num_accts,num_good,num_bad,target_rate],axis=1)
            ...
            if YXX_missing.empty==False:
                ...
                m_num_bad=round(YXX_missing[YXX_missing['target']==1]['weight'].sum(),1)
                m_num_good=round(YXX_missing[YXX_missing['target']==0]['weight'].sum(),1)
                
                #m_num_bad=0 if m_num_bad is None
                #m_num_good=0 if m_num_good is None

                m_target_rate=m_num_bad/m_num_accts
                missing=pd.DataFrame(np.array([[np.nan,np.nan,m_num_accts,m_num_good,m_num_bad,m_target_rate]]),
            ...
            if YXX_special.empty==False:   
                ...
                s_num_bad=round(YXX_special[YXX_special['target']==1]['weight'].sum(),1)
                s_num_good=round(YXX_special[YXX_special['target']==0]['weight'].sum(),1)
                
                #s_num_bad=0 if s_num_bad is None
                #s_num_good=0 if s_num_good is None
                    
                s_target_rate=s_num_bad/s_num_accts
                special=pd.DataFrame(np.array([[s_min,s_max,s_num_accts,s_num_good,s_num_bad,s_target_rate]]),
                                     columns=['min','max','#accts','#good','#bad','target_rate'])
            ...
            final_bin['iv']=(final_bin['dist_bad']-final_bin['dist_good'])*final_bin['woe']
            ...
            final_bin=final_bin[['attr','min','max','#accts','%accts','#good','#bad',
                                 'dist_good','dist_bad','target_rate','woe','iv']]
    ...
    def transform(self,X,train_data=0,keep=False):
        ...
        if iv_remove:
            print(len(iv_remove)," attributes below will be removed form transfomration beacause their information value is below threshold:")
            print(iv_remove)
            print()
            print()
        ...
        for num in num_list:
            ...
            woe_value_list=woe_value[woe_value["index"].isin(["special","missing"])==False]['woe'].to_list()
            
            if len(self.cutoffs[num]) == len(woe_value_list)+1:
                cutoff_list=self.cutoffs[num]
            else:
                cutoff_list=[float("-inf")]+woe_value[woe_value["index"].isin(["special","missing"])==False]['max'].to_list()[:-1]+[float("inf")]
            
            
            XX[num+"_woe"]=pd.cut(XX[num],cutoff_list,right=True,labels=woe_value_list)
            XX[num+"_woe"]=XX[num+"_woe"].astype(float)
            
            XX.loc[XX[num].isin(self.special[num]),num+"_woe"]=woe_value[woe_value["index"]=="special"]['woe'].values
            
            if len(woe_value[woe_value["index"]=="missing"])>0:
                XX.loc[XX[num].isnull(),num+"_woe"]=woe_value[woe_value["index"]=="missing"]['woe'].values.item()
            else:
        ...
        if train_data==0:
            for name in num_list:          
                ...
                t_dist_df=pd.concat([t_dist_min,t_dist_max,t_dist_count,t_dist_pct],axis=1)
                ...
                t_dist_pct_new=t_dist_df.merge(self.woe[name],how='outer',left_on='woe',right_on='woe')
                t_dist_pct_new['%accts']=t_dist_pct_new['%accts'].str.rstrip('%').astype('float') / 100.0
                t_dist_pct_new.loc[t_dist_pct_new['new_dist'].isnull(),'new_dist']=0.0001
                t_dist_pct_new.loc[t_dist_pct_new['%accts']==0,'%accts']=0.0001
                t_dist_pct_new['psi'] = round((t_dist_pct_new['new_dist']-t_dist_pct_new['%accts'])*np.log(t_dist_pct_new['new_dist']/t_dist_pct_new['%accts']),5)     
                t_dist_pct_new.rename(columns={"%accts": "orig_dist"},inplace=True)
                t_dist_pct_new['orig_dist']=t_dist_pct_new['orig_dist'].map('{:,.2%}'.format)
                t_dist_pct_new['new_dist']=t_dist_pct_new['new_dist'].map('{:,.2%}'.format)
                t_dist_pct_new=t_dist_pct_new[['min','max','orig_dist','new_dist','#accts','psi']].sort_values(by='min').reset_index().drop(columns='index')
            ...
            for name in char_list:
                t_dist_pct=pd.DataFrame(XX.groupby(name+'_woe')[name].count()/len(XX))
                ...
                t_dist_pct_new=t_dist_pct.merge(woe_char_new,how='outer',left_on='woe',right_on='woe')
                ...
                t_dist_pct_new.rename(columns={"%accts": "orig_dist"},inplace=True)
                self.psi[name]=round(t_dist_pct_new['psi'].sum(),5)
                self.psi_detail[name]=t_dist_pct_new[[name,'woe','orig_dist','new_dist','psi']].sort_values(by='orig_dist').reset_index() 
    ...
    def display_bin_results(self,attrs_list=None,simple=1,out=None):
        ...
        if simple!=1:
            output_detail_list=[ 'attr', 'min','max','#accts', '%accts', '#good', '#bad', 'dist_good', 'dist_bad', 'target_rate', 'woe','iv']
        else:
            output_detail_list=[ 'attr', 'min','max','#accts', '%accts', '#bad', 'target_rate']
    ...
    def transform_from_json(self,X,json_file,attr_list=None,keep=False):
        ...
        for attr in final_list:
            ...
            if dict_woe[attr]['type']=='num':
                ...
                XX[attr+"_woe"]=pd.cut(XX[attr],cutoff_list,right=True,labels=woe_value_list)
                XX[attr+"_woe"]=XX[attr+"_woe"].astype(float)
                
                XX.loc[XX[attr].isin(dict_woe[attr]['special']),attr+"_woe"]=woe_value[woe_value["index"]=="special"]['woe'].values
                
                if len(woe_value[woe_value["index"]=="missing"])>0:
                    XX.loc[XX[attr].isnull(),attr+"_woe"]=woe_value[woe_value["index"]=="missing"]['woe'].values
                else:
