...
$.extend( Buttons.prototype, {
	...
	processing: function ( node, flag )
	{
		...
		if (flag==true) {
				$(button.node).addClass("processing");
				//node.innerHTML='TRUE';
		}
		else {
				$(button.node).removeClass("processing");
				//node.innerHTML='FALSE';
		}

		//$(button.node).text( flag);
//var clicked_element=document.getElementById(clicked_id);

		//$(button.node).addClass( this.c.dom.button.disabled );

/*
		$(button.node).addClass("processinga");
		$(button.node).css("display","none");
		$(button.node).hide("display","none");

document.getElementById('sidebar').style.display='none';
document.getElementById('sidebar').style.offsetHeight;
//document.getElementById('sidebar').style.display='';

//		setTimeout(function() {$('#sidebar').hide();}, 0);
	*/
